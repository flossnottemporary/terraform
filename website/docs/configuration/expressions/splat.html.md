...
Splat expressions have their iteration effect for lists, sets, and tuples only.
To get a similar result with a map or object value you must use
[`for` expressions](for.html), which are a more general construct.

Resources that use the `for_each` argument appear in expressions as a map of
objects, so you can't use splat expressions with such resources.
